...
class PantallaPrincipal(Tk):
    def __init__(self, main):
        ...
        self.toolbar = Toolbar(self, R.BG)

        self.marco = Frame(self, bg=R.BG, pady=50)
        self.marco.pack(fill='both', expand=1)

        self.lblBoton = Label(self.marco, image=self.IMG_BUTTON_ON, bg=R.BG)
        self.lblBoton.bind('<Button-1>', self.onOffButtonClic)
        self.lblBoton.pack(anchor='center')

        self.lblEspacio = Label(self.marco, bg=R.BG)
        self.lblEspacio.pack()

        self.lblMensaje = Label(self.marco, font=("", "22"), bg=R.BG, fg="white")
